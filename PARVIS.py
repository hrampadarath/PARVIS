...
def main(inFile):
    ...
    primary_beam = pb + control['primary_beam'][0]
    ...
    tbl_uv_data = config_uv_data(params, tbl_uv_data, OBSERVATORY, source, pointing, trange, freq_params, advanced,
                                 array_config, src_list, t_len,primary_beam)
...
def generate_visibitites(uvws, elapsed, julian_midnight, array, params, pointing, freq_params, advanced, src_list,
                         t_len, primary_beam):
    """
    ...
    if doPB == 'T':
        ...
        xyJones = makeJonesMatrix(fov, freq, cell_size,primary_beam)
        x = xyJones[0]
        y = xyJones[1]
        Jones = xyJones[2:]
    ...
    for ti in range(int(t_len)):
        ...
        for c in range(int(chan_len)):
            ...
            for p in range(int(pol_len)):
                ...
                i = 0
                for s in srclist:
                    visa, vp, pa = sourceVisibility(s, RAc, DECc, pol, Mterms, basel_uvw, ha, pointing['lat'], advanced,
                                                    freq_scale)
                    vis.append(visa)
                    i += 1
                    vp_all[ti, p] = vp
...
def config_uv_data(params, tbl_uv_data, antenna_array, source, pointing, trange, freq_params, advanced, array_config,
                   src_list, t_len, primary_beam):
    t_len = params[0]
    chan_len = params[1]
    pol_len = params[2]
    bl_len = params[3]
    ...
    uvdata = generate_visibitites(uvws, elapsed, julian_midnight, antenna_array, params, pointing, freq_params,
                                  advanced, src_list, t_len,primary_beam)
