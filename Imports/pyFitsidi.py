...
def make_primary(config='config.xml'):
  ...
  for key in primary: print 'key = {}, primary[key] = {} \n'.format(key, primary[key])
  #sys.exit()

  #for key in primary: hdu.header.update((key, primary[key]))
  for key in primary: hdu.header[key] = primary[key]
  #for key in common: hdu.header.update((key, common[key]))
  for key in common: hdu.header[key] = common[key]
...
def make_array_geometry(config='config.xml', num_rows=1):
  ...
  * DIAMETER: Antenna diameter (optional)
  ...
  for key in array_geometry: tblhdu.header[key] = array_geometry[key]
  for key in common: tblhdu.header[key] = common[key]
...
def make_antenna(config='config.xml', num_rows=1):
  ...
  * POLCALB:  As above, for feed B
  ...
  for key in cards: tblhdu.header[key] = cards[key]
  for key in common: tblhdu.header[key] = common[key]
...
def make_uv_data(config='config.xml', num_rows=1):
  ...
  * FLUX         UV visibility data matrix
  ...
  for key in sorted(common): tblhdu.header[key] = common[key]
  for key in sorted(cards): tblhdu.header[key] = cards[key]
